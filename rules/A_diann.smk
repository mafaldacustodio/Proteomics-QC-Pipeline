...
rule convert_raw_files:
    input:
        raw = "/projects/cbmr_fpm_soup-AUDIT/data/pipeline/data_input/raw_files/{sample}.raw"
    output:
...
rule convert_all:
    input:
        [f"/projects/cbmr_fpm_soup-AUDIT/data/pipeline/data_input/converted_files/{row['workflow']}/{row['cohort']}/{row['file_name']}.mzML"
