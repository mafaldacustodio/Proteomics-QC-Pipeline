...
# 1. Setup
# ---------------------------------------------------------------------
configfile: "config.yaml"  # user defines 'project_root' inside this file
directory_root = Path(config["project_root"]).expanduser().resolve()
...
rule convert_to_htrms:
    ...
    """
    input:
        raw=lambda wc: directory_root / "data_input/raw_files" / f"{wc.sample}{get_ext(wc.sample)}"
    output:
        htrms=lambda wc: directory_root / "data_input/converted_files" / wc.workflow / wc.cohort / f"{wc.sample}.htrms"
    params:
        outdir=lambda wc: directory_root / "data_input/converted_files" / wc.workflow / wc.cohort
    threads: 4
    resources:
        mem_mb=64000,
...
rule A_all:
    ...
    output:
        marker = f"{directory_root}/data_output/A_all_spectronaut_complete.marker"
    shell:
        "touch {output.marker}"
